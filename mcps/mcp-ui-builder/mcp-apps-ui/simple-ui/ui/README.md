# MCP Apps UI 模板（simple-ui）

simple_tool 工具的 UI 项目。**单工具单项目**：`src/App.tsx` 直接渲染 `SimpleResultPage`，无工具名路由（无 `PAGES` / `resolveToolName` / `window.__MCP_TOOL_NAME__`）。

## 适用范围

本模板适用于：**MCP 工具需要在宿主(如 Claude Desktop)中渲染交互式界面**的场景。这是三种复杂度中最简单的一种——工具执行快、同步返回结构化结果、无需进度推送、无需人工审核。

> 同类还有两个兄弟模板项目（按需选用）：
> - `mcp-apps-ui/progress-ui/` — 进度推送 + 结果展示
> - `mcp-apps-ui/review-ui/`   — 进度 + 人工审核 + 提交

> **使用模型：一个工具 = 一个独立项目 = 一个 `ui://` 资源。** 宿主执行一个工具时创建 iframe 展示该工具的 view，所以每个工具用独立的 ui 项目，`App.tsx` 直接渲染该工具的页面。服务端为该工具绑定一个独立的 `ui://` 资源（见 `../server_example/server.py` 中 `simple_tool` 的注册方式）。
...
```bash
cd mcp-apps-ui/simple-ui/host
npm install
npm run dev          # 打开 http://localhost:5184
```

在控制台点击"自动播放"，即可看到完整流程。

### 3. 启动服务端示例（可选）

```bash
pip install fastmcp>=3.4.4
cd mcp-apps-ui       # 在 mcp-apps-ui 根目录执行，server_example 与 simple-ui 同级
python -m server_example.server    # http://127.0.0.1:8000
```

服务端示例演示了如何注册 UI 资源、推送进度、实现审核阻塞。详见 `../server_example/README.md`。

## 目录结构

```
mcp-apps-ui/
├── simple-ui/                    # 本项目（simple_tool 专用 UI）
│   ├── ui/                       # MCP Apps UI 前端
│   │   ├── src/
│   │   │   ├── core/                # 通用层(请勿修改)
...
│   │   │   ├── pages/               # 业务页面(本模板只有 SimpleResultPage)
│   │   │   │   └── SimpleResultPage.tsx
│   │   │   ├── hooks/useTheme.ts    # 主题响应(根据 hostContext.theme 切换 .dark)
│   │   │   └── App.tsx              # 根组件(直接渲染 SimpleResultPage，无路由)
│   │   └── README.md                # 本文件
│   └── host/                     # 简化版测试控制台
...
在 `src/pages/SimpleResultPage.tsx` 中定义你的工具返回结构：
...
把 `SimpleResultPage.tsx` 改名为 `YourPage.tsx`（或直接在原文件改），替换：
- 结果类型(`SimpleResult` -> `YourResult`)
- 渲染逻辑(展示你的业务数据)

### Step 3: 改 App.tsx 指向你的页面

`src/App.tsx` 默认 import `SimpleResultPage`，改名后更新 import 即可：
...
// 渲染处：<SimpleResultPage /> 改成 <YourPage />
```
...
编辑 `../host/src/mockData.ts`，把工具名改成你的，把 `finalResult` 改成你的工具真实返回结构。

### Step 5: 服务端注册 UI 资源

在 MCP server 中为该工具注册独立的 `ui://` 资源并绑定（参考 `../server_example/server.py` 中 `simple_tool` 的写法）：
...
2. **审核卡在"执行中"** -> 后端没推 `uiEvent.review_pending`，见"后端协议约定"。
3. **`useEditedData` 的 initial** -> 已内置深比较防护，每次渲染传新对象字面量也不会死循环。但为减少每次渲染的深比较开销，建议用 `useMemo` 保持引用稳定：
   ...
   ```
4. **页面显示"未知工具"** -> 工具名未在 `src/App.tsx` 的 `PAGES` 中配置：检查工具名拼写，或补充映射。
...
- **步骤 5**：host 发送 `review_pending`(含 final_result + review_id)，UI 进入审核态
- **步骤 6**：用户点击"通过/驳回"，UI 反向调用 `submit_review` 工具
- **步骤 7**：host 发送 tool-result(最终结果)，UI 显示完成

核心：所有数据通过 postMessage 流转，无 HTTP 端点。UI 用 `useMcpApp()` 订阅状态，用 `useNormalizedToolResult()` 获取结果，用 `submitReview()` 提交审核。
