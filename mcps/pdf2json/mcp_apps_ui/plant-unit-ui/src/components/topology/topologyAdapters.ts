...
export function plantUnitToRF(drawing: PlantUnitDrawing, imageUrl?: string): { nodes: Node[]; edges: Edge[] } {
  ...
  for (const raw of drawing.nodes ?? []) {
    ...
    if (!id) continue;
    // 优先显式 x/y；缺失时从 position(像素中心)/bbox 兜底（对齐 PlantUnitNode 真实载荷）
    let x = Number(raw.x ?? 0);
    let y = Number(raw.y ?? 0);
    let w = Number(raw.width ?? 100);
    let h = Number(raw.height ?? 60);
    if (raw.x == null && raw.y == null && Array.isArray(raw.position) && raw.position.length === 2) {
      x = Number(raw.position[0]) - w / 2;
      y = Number(raw.position[1]) - h / 2;
    } else if (raw.x == null && raw.y == null && Array.isArray(raw.bbox) && raw.bbox.length === 4) {
      x = Number(raw.bbox[0]);
      y = Number(raw.bbox[1]);
      w = Number(raw.bbox[2]) - Number(raw.bbox[0]);
      h = Number(raw.bbox[3]) - Number(raw.bbox[1]);
    }
    const data: RFNodeData = {
