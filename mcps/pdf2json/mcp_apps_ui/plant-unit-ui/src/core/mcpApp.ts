  ...
  private handleNotification(method: string, params: unknown): void {
    ...
    switch (method) {
      ...
      case 'notifications/progress': {
        ...
        // 累积合并 partial_plant_units（按 id 去重，新数据覆盖旧数据）
        const mergedPlantUnits = mergeById(
          this.store.progress?.uiEvent?.partial_plant_units,
          incomingUiEvent?.partial_plant_units,
        );
        // 累积合并 partial_text_destinations（按 id 去重）
        const mergedTextDests = mergeById(
          this.store.progress?.uiEvent?.partial_text_destinations,
          incomingUiEvent?.partial_text_destinations,
        );
        // 累积合并 partial_edges（按 source_node_id|target_node_id 去重）
        const mergedEdges = mergeEdges(
          this.store.progress?.uiEvent?.partial_edges,
          incomingUiEvent?.partial_edges,
        );
        ...
        this.store = {
          ...
          progress: {
            ...
            uiEvent: {
              ...
              partial_plant_units: mergedPlantUnits,
              partial_text_destinations: mergedTextDests,
              partial_edges: mergedEdges,
            },
          },
...
/** 按唯一键去重合并 two 段记录数组：新数据覆盖旧数据，顺序保持先旧后新。 */
function mergeById(
  prev: unknown,
  incoming: unknown,
): Array<Record<string, unknown>> {
  const existing = Array.isArray(prev) ? (prev as Array<Record<string, unknown>>) : [];
  const adding = Array.isArray(incoming)
    ? (incoming as Array<Record<string, unknown>>)
    : [];
  const byId = new Map<string, Record<string, unknown>>();
  for (const item of existing) byId.set(String(item.id), item);
  for (const item of adding) byId.set(String(item.id), item);
  return Array.from(byId.values());
}

/** 按 source_node_id|target_node_id 去重合并边列表。 */
function mergeEdges(
  prev: unknown,
  incoming: unknown,
): Array<Record<string, unknown>> {
  ...
  for (const e of existing) byKey.set(`${e.source_node_id}|${e.target_node_id}`, e);
  for (const e of adding) byKey.set(`${e.source_node_id}|${e.target_node_id}`, e);
...
function inferEventType(
  ...
  // 2. 服务端显式声明的事件类型（如 images_loaded / drawing_type_classified）
