 ...
 * 提取进度面板：流式展示提取执行过程。
 *
 * 在审核数据就绪前（status 非 waiting_review/confirming/completed/error），
 * 替代"等待提取完成…"占位，显示：
 *   - 当前状态大图标 + 动画
 *   - SSE 事件时间线（流式滚动，最新在底部）
 *   - 错误详情（status=error 时）
 *
...
const EVENT_LABEL: Record<string, string> = {
  ...
  pdf_received: 'PDF 已接收，解析中',
  pdf_parsed: 'PDF 解析完成',
  extraction_started: '开始提取',
  drawing_info_extracted: '图纸信息已提取',
  equipment_nodes_extracted: '设备节点已提取',
  boundary_nodes_extracted: '边界节点已提取',
  topology_extracted: '拓扑已提取',
  nodes_extracted: '节点已提取',
  extraction_completed: '提取完成',
  chapter_extracted: '章节抽取完成',
  expert_complete: '专家提取完成',
  pd_topology_extraction_started: '工艺说明拓扑提取开始',
  pd_topology_extracted: '工艺说明拓扑已提取',
  stage_update: '阶段更新',
...
export function ExtractionProgress({}: ExtractionProgressProps) {
  const app = useMcpApp();
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      {/* 顶部：当前状态 */}
      <div className="border-b border-border bg-bg-2/40 px-6 py-6">
      ...
      {/* 错误详情 */}
      {isError && app.error && (
        <div className="max-h-40 overflow-auto border-t border-border bg-red/5 p-4">
...
function EventRow({ event, isLast }: { event: SessionEvent; isLast: boolean }) {
  ...
  // 额外信息（如匹配章节数、文本长度、专家结果等）
  const extraParts: string[] = [];
  const d = event.data ?? {};
  for (const key of [
    'page_count', 'node_count', 'edge_count', 'count', 'pages',
    'matched_section_count', 'total_pages', 'text_length',
    'has_data', 'success', 'confidence',
  ]) {
    const v = d[key];
    if (typeof v === 'number' || typeof v === 'string' || typeof v === 'boolean') {
      extraParts.push(`${key}=${v}`);
    }
  }
