  ...
  const toast = toasts.length > 0 && (
    <div className="pointer-events-none fixed right-6 top-16 z-50 flex flex-col gap-2">
      {toasts.map((t) => (
        <div
          key={t.id}
          className={`pointer-events-auto flex cursor-pointer items-center gap-2 rounded-md px-4 py-2.5 text-sm text-white shadow-card transition hover:scale-[1.02] active:scale-[0.98] ${
            t.type === 'success'
              ? 'bg-green/95'
              : t.type === 'error'
                ? 'bg-red/95'
