    ...
    <>
      {trigger
        ...
        : (
          <button
            ...
            title="查看 JSON"
            onClick={handleOpen}
            className="fixed right-4 top-4 z-30 flex h-9 w-9 items-center justify-center rounded-lg border border-border bg-bg-2/80 text-text-2 shadow-card backdrop-blur-md hover:border-accent hover:text-accent"
          >
      ...
      {mounted && (
        <div className={`${overlayPos} z-40 flex flex-col justify-end`} onClick={handleClose}>
          ...
          <div
            className={`relative max-h-[70%] overflow-auto rounded-t-lg border-t border-border bg-bg shadow-card transition-transform duration-300 ease-out ${open ? 'translate-y-0' : 'translate-y-full'}`}
            onClick={(e) => e.stopPropagation()}
          >
            <div className="sticky top-0 flex items-center justify-between border-b border-border bg-bg-2/90 px-4 py-2 backdrop-blur">
              <span className="text-xs font-bold text-text">原始 JSON</span>
              <div className="flex items-center gap-2">
                <button
                  onClick={handleExport}
                  title="导出为 JSON 文件"
                  className="flex items-center gap-1 rounded border border-border bg-bg-3/50 px-2 py-0.5 text-[10px] text-text-2 transition hover:bg-bg-3 hover:text-text"
                >
