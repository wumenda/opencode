 ...
 *
 * 优先级：
 *   1. 业务模式注册的解析器（见 registerProgressEventResolver）
 *   2. 后端推送的显式 `event_type` 字段
 *   3. 拓扑工具默认字段反推（兼容 mock 旧数据）
 */
function inferEventType(
  ...
  // 2. 后端推送的显式 event_type 字段（新协议）
  const explicitType = uiEvent?.event_type as string | undefined;
  if (explicitType) return explicitType;
  // 3. 字段反推兜底（兼容未带 event_type 的旧推送/mock 数据）
