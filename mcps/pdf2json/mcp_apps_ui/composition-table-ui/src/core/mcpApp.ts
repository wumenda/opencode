  ...
  private handleNotification(method: string, params: unknown): void {
    ...
    switch (method) {
      ...
      case 'notifications/progress': {
        ...
        // 累积合并 partial_extracted：components 按名称去重，streams 全部追加
        const incomingPartialExtracted = incomingUiEvent?.partial_extracted as
          | { components?: unknown[]; streams?: unknown[] }
          | undefined;
        const prevPartialExtracted = (
          this.store.progress?.uiEvent?.partial_extracted as
            | { components?: unknown[]; streams?: unknown[] }
            | undefined
        ) ?? { components: [], streams: [] };
        const mergedPartialExtracted = incomingPartialExtracted
          ? mergePartialExtracted(
              prevPartialExtracted,
              incomingPartialExtracted,
            )
          : prevPartialExtracted;

        // partial_aggregated 直接替换（最终聚合结果）
        const mergedPartialAggregated = incomingUiEvent?.partial_aggregated
          ? incomingUiEvent.partial_aggregated
          : this.store.progress?.uiEvent?.partial_aggregated;
        ...
        this.store = {
          ...
          progress: {
            ...incoming,
            uiEvent: {
              ...this.store.progress?.uiEvent,
              ...incomingUiEvent,
              partial_page_graphs: mergedGraphs,
              image_paths: mergedImagePaths,
              image_infos: mergedImageInfos,
              partial_extracted: mergedPartialExtracted,
              partial_aggregated: mergedPartialAggregated,
            },
          },
...
/**
 * 累积合并 partial_extracted（跨页）：components 按名称去重，streams 全部追加。
 * 与后端 CompositionTableWorkflow.aggregate 同款逻辑。
 */
function mergePartialExtracted(
  prev: { components?: unknown[]; streams?: unknown[] },
  incoming: { components?: unknown[]; streams?: unknown[] },
): { components: unknown[]; streams: unknown[] } {
  const existingComps = Array.isArray(prev.components) ? prev.components : [];
  const newComps = Array.isArray(incoming.components) ? incoming.components : [];
  const newStreams = Array.isArray(incoming.streams) ? incoming.streams : [];
  const seen = new Set(existingComps.map((c) => String((c as { name?: string }).name)));
  const mergedComps = [...existingComps];
  ...
  return {
    ...
    streams: [...(Array.isArray(prev.streams) ? prev.streams : []), ...newStreams],
  };
}
...
function inferEventType(
  ...
  // 2. Server 显式事件类型（如 composition_page_extracted / aggregation_complete）
